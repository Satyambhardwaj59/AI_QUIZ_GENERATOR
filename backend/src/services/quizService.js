...
export async function extractFromFile(file) {
 ...
 if (ext === ".pdf") {
   ...
   // Clean and normalize text - handle single words
 ...
 if (ext === ".docx") {
   ...
   // Clean and normalize text - handle single words
   const extractedText = data.value || "";
   return extractedText.trim().replace(/\s+/g, " ");
 }
 ...
 if ([".png", ".jpg", ".jpeg"].includes(ext)) {
   try {
     // Read file as buffer for better serverless compatibility
     const imageBuffer = fs.readFileSync(file.path);
    
     // Configure Tesseract for serverless environments
     // Use buffer-based recognition instead of file path (more reliable in serverless)
     const result = await Promise.race([
       Tesseract.recognize(imageBuffer, "eng", {
         logger: (m) => {
           // Only log errors in development
           if (process.env.NODE_ENV === "development" && m.status === "error") {
             console.error("Tesseract error:", m);
           }
         },
       }),
       // Add timeout for serverless (60 seconds max - Vercel has 60s timeout)
       new Promise((_, reject) =>
         setTimeout(() => reject(new Error("OCR processing timed out. Please try with a smaller or clearer image.")), 55000)
       ),
     ]);
    
     // Clean and normalize the extracted text - handle single words
     const extractedText = result.data.text || "";
     // Remove extra whitespace but preserve single words
     ...
     if (ocrError.message && ocrError.message.includes("No text")) {
       throw ocrError;
     }
     if (ocrError.message && ocrError.message.includes("timed out")) {
       throw ocrError;
     }
     // Check for common serverless issues
     if (ocrError.message && (ocrError.message.includes("ENOENT") || ocrError.message.includes("worker"))) {
       throw new Error("OCR service initialization failed. Please try again or use a different image format.");
     }
     throw new Error(`Failed to extract text from image: ${ocrError.message || "OCR processing failed"}`);
   }
 }
...
export async function generateQuizWithAI({ text, difficulty, numQuestions, allowImages }) {
 ...
 const trimmedText = text.trim().slice(0, 6000);
  // Check if content is very short (single word or minimal text)
 const isMinimalContent = trimmedText.length <= 50 && trimmedText.split(/\s+/).length <= 3;
...
`;
 ...
 // Enhanced prompt for minimal content (single words)
 let userPrompt = "";
 if (isMinimalContent) {
   userPrompt = `
Source content (minimal text - single word or short phrase):
"${trimmedText}"


IMPORTANT: Since the source content is minimal (${trimmedText.length} character(s)), generate creative and educational questions about this word/phrase.
You can create questions about:
...
- Characteristics or properties (if applicable)


Generate exactly ${clampedNum} multiple-choice questions. Each question must have exactly 4 options and exactly one correct answer.
Be creative and educational - expand on the minimal content to create meaningful quiz questions.
